...
export default function TwoPartJokes() {
  const [params, setParams] = useState("");
  const [data, setData] = useState([]);

  const [{ response, error, isLoading }] = FetchJokes(
    `${API_URL}${params}?type=twopart&amount=10&blacklistFlags=nsfw,racist,sexist,explicit`
  );
        ...
        <div className="drop__container" id="drop-items">
          ...
          <div className="container">
            {isLoading ? (
              ...
              data.map((j, i) => (
                <Joke key={i} setup={j.setup} id={j.id} delivery={j.delivery} />
              ))
            )}
          </div>
